...
/**
 * Erzeugung eines Songs aus Blöcken greift auf Song-Daten zu und kann
...
public class SongManager {
    ...
    /**
     * Konstruktor Lädt alle gespeicherten Songs aus der Datei
     */
    public SongManager() {
        builtSong = null;
        editSong = null;
    ...
    /**
     * Block wird aus der Liste entfernt, Koordinaten aller verbleibenden Blöcke neu
     * berechnet
     * 
    ...
    /**
     * Alle Blöcke verwerfen Counter zum Errechnen der Koordinaten auf
    ...
    public Song confirm(String name) throws NameException {
        if (name == null || name.equals("")) {
            ...
            builtSong = new Song();
            builtSong.setTones(convertToTones());
            builtSong.setLevel(calcLevel(builtSong.getTones()));
            builtSong.setEditable(true);

            if (songPersitinator.nameAccepted(name)) {
                builtSong.setName(name);
                addSong(builtSong);
            }
            ...
            // bestehenden Song überschrieben, wenn im Editiermodus
                ...
                savedSongs.clear();
                builtSong.setName(editSong.getName());
                savedSongs.addAll(songPersitinator.overrideData(builtSong));
                editSong = null;
            }
    ...
    public Song getBuiltSong() {
        return builtSong;
    }

    /**
     * Errechnet das Level für einen erstellten Song anhand der maximalen Distanz
     * und Anzahl der Töne
     * 
    ...
    public Level calcLevel(List<Tone> tones) {
        ...
        if (inputBlocks.size() < 25 && maxdist < 5)
            return Level.BEGINNER;
        if (inputBlocks.size() < 70 && maxdist < 10)
    ...
    public void addSong(Song song) {
        songPersitinator.saveData(builtSong);
        savedSongs.clear();
        savedSongs.addAll(songPersitinator.loadAll());
    }

    /**
     * Entfernt einen Song aus Datei und aktualisiert die Liste mit verwalteten
     * Songs
     * 
